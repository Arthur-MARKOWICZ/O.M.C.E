...
export default function TabsLayout() {
  const { session, colors, cart, isVendedor, isComprador } = useApp(); if (!session) return <Redirect href="/(auth)/login" />;
  const cartButton = isComprador ? <Link href="/carrinho" asChild><Pressable accessibilityLabel="Carrinho" style={{ marginRight: 16 }}><Ionicons name="cart-outline" size={24} color={colors.ink} />{cart.length > 0 && <View style={{ position: 'absolute', top: -7, right: -10, minWidth: 16, height: 16, borderRadius: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.lime }}><Text style={{ fontSize: 9, fontFamily: 'DMMono_500Medium', color: colors.ink }}>{cart.length}</Text></View>}</Pressable></Link> : undefined;
