...
export default function Layout({ children }) {
  ...
  return <><header className="topbar"><Link className="brand" to="/"><span className="brand-mark">◈</span><span>O.M.C.E</span></Link><nav>{nav.map(([to, label]) => <NavLink key={to} to={to} end={to === '/'}>{label}</NavLink>)}</nav><div className="top-actions"><ThemeToggle theme={theme} onToggle={toggleTheme} />{canBuy && <Link className="cart-button" to="/carrinho" aria-label="Carrinho">🛒<b>{cartSize}</b></Link>}<details className="account-menu"><summary>{auth.name || 'Minha conta'}</summary><div>{canSell && <Link to="/meus-produtos">Meus produtos</Link>}{canBuy && <Link to="/historico/compras">Minhas compras</Link>}{canSell && <Link to="/historico/vendas">Minhas vendas</Link>}{canSell && <Link to="/avaliacoes">Minhas avaliações</Link>}<button onClick={logout}>Sair</button></div></details></div></header><main>{children}</main></>;
}
